/***************************************************************************
  ...
  Class for acceleration of the lj/cut/bump pair style.
...
const char *lj_bump=0;
#else
#include "lj_bump_cubin.h"
#endif

#include "lal_lj_bump.h"
#include <cassert>
namespace LAMMPS_AL {
#define LJBumpT LJBump<numtyp, acctyp>

extern Device<PRECISION,ACC_PRECISION> device;

template <class numtyp, class acctyp>
LJBumpT::LJBump() : BaseAtomic<numtyp,acctyp>(), _allocated(false) {
}
...
template <class numtyp, class acctyp>
int LJBumpT::bytes_per_atom(const int max_nbors) const {
  return this->bytes_per_atom_atomic(max_nbors);
}

template <class numtyp, class acctyp>
int LJBumpT::init(const int ntypes,
...
template <class numtyp, class acctyp>
void LJBumpT::reinit(const int ntypes, double **host_cutsq, double **host_lj1,
                 double **host_lj2, double **host_lj3,
                 double **host_lj4, double **host_offset,
...
template <class numtyp, class acctyp>
double LJBumpT::host_memory_usage() const {
  return this->host_memory_usage_atomic()+sizeof(LJBump<numtyp,acctyp>);
}
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void LJBumpT::loop(const bool _eflag, const bool _vflag) {
  ...
  if (shared_types) {
    ...
    this->k_pair_fast.run(&this->atom->x, &lj1, &lj3, &sp_lj, &bump_data,
...
template class LJBump<PRECISION,ACC_PRECISION>;
}
