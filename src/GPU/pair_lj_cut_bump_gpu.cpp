...
void ljb_gpu_clear();
int ** ljb_gpu_compute_n(const int ago, const int inum,
...
void ljb_gpu_compute(const int ago, const int inum, const int nall,
...
PairLJCutBumpGPU::PairLJCutBumpGPU(LAMMPS *lmp) : PairLJCutBump(lmp), gpu_mode(GPU_FORCE)
{
  respa_enable = 0;
  cpu_time = 0.0;
...
PairLJCutBumpGPU::~PairLJCutBumpGPU()
{
  ljb_gpu_clear();
}
...
void PairLJCutBumpGPU::init_style()
{
  cut_respa = nullptr;
...
void PairLJCutBumpGPU::reinit()
{
  Pair::reinit();

  ljb_gpu_reinit(atom->ntypes+1, cutsq, lj1, lj2, lj3, lj4, offset,
